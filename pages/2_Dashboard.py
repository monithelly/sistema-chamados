...
if df.empty:
    ...
    chamados_por_criticidade = df["criticidade"].value_counts().reset_index()
    ...
    with col1:
        st.markdown(
            f"""<div style="background-color:#132A63;padding:20px;border-radius:16px;text-align:center;color:white;min-height:130px;display:flex;flex-direction:column;justify-content:center;">
                <div style="font-size:18px;font-weight:600;color:white;">Total</div>
                <div style="font-size:42px;font-weight:700;color:#7EDC12;margin-top:10px;">{total_chamados}</div></div>""",
            unsafe_allow_html=True
        )

    with col2:
        st.markdown(
            f"""<div style="background-color:#EAF0FA;padding:20px;border-radius:16px;text-align:center;min-height:130px;display:flex;flex-direction:column;justify-content:center;">
                <div style="font-size:18px;font-weight:600;color:#132A63;">Aguardando</div>
                <div style="font-size:42px;font-weight:700;color:#C79200;margin-top:10px;">{aguardando}</div></div>""",
            unsafe_allow_html=True
        )
    ...
    with col3:
        st.markdown(
            f"""<div style="background-color:#EAF0FA;padding:20px;border-radius:16px;text-align:center;min-height:130px;display:flex;flex-direction:column;justify-content:center;">
                ...
                <div style="font-size:42px;font-weight:700;color:#1E88E5;margin-top:10px;">{aberto}</div></div>""",
            unsafe_allow_html=True
        )
    ...
    with col4:
        st.markdown(
            f"""<div style="background-color:#EAF0FA;padding:20px;border-radius:16px;text-align:center;min-height:130px;display:flex;flex-direction:column;justify-content:center;">
                ...
                <div style="font-size:42px;font-weight:700;color:#2E7D32;margin-top:10px;">{finalizado}</div></div>""",
            unsafe_allow_html=True
        )
    ...
    if not df_finalizados.empty:
        ...
        st.markdown(
            f"""<div style="background-color:#DFF5BF;padding:18px;border-radius:16px;text-align:center;color:#132A63;margin-bottom:25px;">
                <div style="font-size:18px;font-weight:600;">⏱ Média de dias para fechamento</div>
                <div style="font-size:34px;font-weight:700;margin-top:8px;">{round(media_tempo, 2)}</div></div>""",
            unsafe_allow_html=True
        )
    else:
